...
run_docker_build_scripts() {
  ...
  local tools_dir="component/common/application/amazon-freertos/tools"
  ...
  # Start the container
  docker run -d --name ameba-container ameba-rtos-aws:ameba-rtos-z2 tail -f /dev/null

  # Execute the setup and build script inside the container
  if ! docker exec ameba-container bash -c "
    chmod u+x ${tools_dir}/docker/amebaz2/amebaz2_build.sh  &&
    ${tools_dir}/docker/amebaz2/amebaz2_build.sh /opt/ameba
  "; then
