...
run_docker_build_scripts() {
  ...
  # Start the container
  docker run -d --name ameba-container ameba-rtos-aws:ameba-rtos-d tail -f /dev/null

  # Execute the setup and build script inside the container
  if ! docker exec ameba-container bash -c "
    chmod u+x ${tools_dir}/docker/amebad/amebad_build.sh  &&
    ${tools_dir}/docker/amebad/amebad_build.sh /opt/ameba
  "; then
